...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "sudo CENTOS_VERSION=$(cut -d: -f5 /etc/system-release-cpe | cut -d. -f1) && yum -y install https://apache.jfrog.io/artifactory/arrow/centos/${CENTOS_VERSION}/apache-arrow-release-latest.rpm && yum -y install arrow-glib-devel-3.0.0 parquet-devel-3.0.0 gcc gcc-c++",
